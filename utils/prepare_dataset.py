    ...
    for i in tqdm(annotations['annotations']):
        if i['category_id']==1:
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['mscoco/{}/person/{}.jpg'.format(set_type, c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        ...
        if i['category_id'] == 2:
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['mscoco/{}/bicycle/{}.jpg'.format(set_type, c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        if i['category_id'] == 3:
            c1 = c1 + 1
            ID = i['image_id']
            zeros = 12 - len(str(ID))
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['mscoco/{}/car/{}.jpg'.format(set_type, c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
    
    print(ann.head())
    ann.to_csv(os.path.join(datasetDir, 'sgada_data/mscoco_{}.txt'.format(set_type, set_type)), header=None, index=None, sep=' ', mode='a')
    ...
    for i in tqdm(annotations['annotations']):
        if i['category_id']==1:
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/train/person/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        if i['category_id'] == 2:
            c1 = c1 + 1
            ID = i['image_id']+1
            zeros = 5 - len(str(ID))
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/train/bicycle/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        if i['category_id'] == 3:
            c1 = c1 + 1
            ID = i['image_id']+1
            zeros = 5 - len(str(ID))
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/train/car/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)

    
    ann.to_csv(os.path.join(datasetDir, 'sgada_data/flir_train.txt'), header=None, index=None, sep=' ', mode='a')
    ...
    for i in tqdm(annotations['annotations']):
        if i['category_id']==1:
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/val/person/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        if i['category_id'] == 2:
            c1 = c1 + 1
            ID = i['image_id']+8863
            if (len(str(ID))<5):
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/val/bicycle/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)
        if i['category_id'] == 3:
            c1=c1+1
            ID = i['image_id']+8863
            if (len(str(ID))<5):
            ...
            if (im_crop[2] > width) or (im_crop[3]  > height) or (im_crop[0]<0) or (im_crop[1]<0):
                ...
                ann = ann.append(pd.DataFrame([['flir/val/car/FLIR_{}.jpeg'.format(c1), i['category_id']-1]],columns=['path','label']),ignore_index=True)

    ann.to_csv(os.path.join(datasetDir, 'sgada_data/flir_val.txt'), header=None, index=None, sep=' ', mode='a')
def main():
