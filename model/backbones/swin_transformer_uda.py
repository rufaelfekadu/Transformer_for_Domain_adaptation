...
class WindowAttention_3_branches(nn.Module):
    ...
    def forward(self, x,x2, use_attn=True, inference_target_only=False, mask=None):
        ...
        if inference_target_only:
            ...
            qkv2 = self.qkv(x2).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) #(3, B_, nh, N, C // nh)
            ...
            # relative_position_bias2 = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
            # relative_position_bias2 = relative_position_bias2.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww

            # relative_position_bias3 = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
            # relative_position_bias3 = relative_position_bias3.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
